...
bookingUpdateView.controller('liveBookingController',['$scope', '$http', '$q',function($scope, $http, $q){
 ...
 $scope.fetchData = function(){
        ...
        }).catch(function onError(response){})//or , function(response)
        .finally(function(){})
 }
...
$scope.promise1 = function(i) {
 ...
 $scope.updateDate = function(id){
    ...
    for (i=0; i<= length; i++){
        if (id.date_play_fk == $scope.list_D_3[i].date_play_fk && isCheck_D_TP==false){console.log($scope.list_D_3[i]);
            $http({
                method:"POST",
 ...
 $scope.updateHallType = function (id){
    ...
    if($scope.generalTheatre && $scope.generalHall && $scope.generalHallType && $scope.generalDate && $scope.generalTime){
        for (i=0; i<= length; i++){
            //mhpws more conditions check HERE>???
...
//console.log($scope.indexSeatsSub +'fffffffff');
                ...
                $scope.list_S_6=[];
                   ...
                   // console.log($scope.list_css);
                //$scope.list_css= $scope.arraySeats[1][0].seatsAvailable; 
                $scope.times_seats_call= Math.ceil($scope.arraySeats[1][0].seatsAvailable/8);
                console.log($scope.times_seats_call);
                //$scope.totalCount = 1;
                //$scope.number=1;
                $scope.rerun=true;
                ...
                console.log($scope.list_css);
                //alert($scope.list_css);
                //$scope.list_css=$scope.times_seats_call;
 ...
 $scope.updateTimePlay = function (id){
    ...
    $scope.isRes = function(id){
        ...
        //for (i=0; i< $scope.$scope.arraySeats[1][0].seatsAvailable; i++) {
    ...
    $scope.keepSeats = function (id) {
        ...
        console.log($scope.keepcountcss);
        return $scope.keepcountcss.toString();
        }
    }
    //$scope.$watch('keepcountcss', function(newValue, oldValue){
        //$scope.keepSeats();
      //  $scope.keepcountcss++;
    //});
